/// Modified by Felix based on UILineRenderer

using System.Collections.Generic;

namespace UnityEngine.UI.Extensions
{
    [AddComponentMenu("UI/Extensions/Primitives/UILineRendererII")]
    [RequireComponent(typeof(RectTransform))]
    public class UILineRendererII : UIPrimitiveBase
	{
        ...
        /// <summary>
        /// The rect area to get line points position
        /// </summary>
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
		{
			if (m_points != null && m_points.Length > 0) {
                ...
                foreach (Transform t in m_points)
                {
                    if (t == null) continue;
                    Vector2 p = Vector2.zero;
                    if (rootRect != null)
                    {
                        RectTransformUtility.ScreenPointToLocalPointInRectangle(rootRect, t.position, null, out p);
                    }
                    points.Add(p);
                    // Debug.Log($"[UILineRendererII] {p.ToString()}");
                }
                // Debug.Log($"[UILineRendererII] {points.Count.ToString()}");
                if (points.Count > 0)
                {
                    PopulateMesh (vh, points.ToArray());
                }
            }
