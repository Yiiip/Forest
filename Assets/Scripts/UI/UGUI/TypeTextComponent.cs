...
public class TypeTextComponent : MonoBehaviour
{
    ...
    public IEnumerator TypeText(string text)
    {
        ...
        for (var i = 0; i < len; i++)
        {
            ...
            if (text[i] == '<' && i + (1 + 15) < len && text.Substring(i, 2 + 6).Equals("<color=#") && text[i + 14] == '>')
            {
                _currentText += text.Substring(i, 15);
                i += 15 - 1;
                symbolDetected = true;
                tagOpened = true;
                tagType = "color";
...
public static class TypeTextComponentUtility
{
    ...
    public static TypeTextComponent GetTypeText(this Text label)
    {
        var typeText = label.GetComponent<TypeTextComponent>();
        if (typeText == null)
        {
